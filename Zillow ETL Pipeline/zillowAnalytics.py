...
with DAG('zillow_analytics_dag',
        ...
        op_kwargs={'url': 'https://zillow56.p.rapidapi.com/search', 'querystring': {"location":"toronto, on"}, 'headers': api_host_key, 'date_string':dt_now_string}
        )

        load_to_s3 = BashOperator(
            task_id = 'tsk_load_to_s3',
            bash_command = 'aws s3 mv {{ ti.xcom_pull("tsk_extract_zillow_data_var")[0]}} s3://zillowRapidAPI-bucket/',
        )
        ...
        aws_conn_id='s3ConnectionAWS',
        redshift_conn_id='redshiftConnectionAWS',
        s3_bucket=s3_bucket,
